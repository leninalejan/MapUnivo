...
router.get('/', authJWT, async (req, res) => {
  try {
    ...
    let q = 'SELECT * FROM zonas WHERE activo=TRUE'
    const params = []
...
router.get('/:slug', authJWT, async (req, res) => {
  try {
    const { rows } = await pool.query(
      'SELECT * FROM zonas WHERE slug=$1 AND activo=TRUE',
      [req.params.slug]
    )
...
router.get('/:slug/salones', authJWT, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      'SELECT * FROM salones WHERE zona_id=$1 AND activo=TRUE ORDER BY piso,codigo',
      [zona.rows[0].id]
    )
    res.json(rows)
  } catch (err) {
